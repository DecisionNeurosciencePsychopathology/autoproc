...
#contains exec_pbs_array and list.dirs -- this might have to be unique in that I just add helperfunctions or move it to my cwd
thorndike_path = "/Users/wilsonj3/Development/autoproc"
#source(normalizePath(file.path(scriptpath, "..", "R_helper_functions.R")))
source(normalizePath(file.path(thorndike_path, "R_helper_functions.R")))
...
#function to rename duplicates i.e. trustx4
renamedups<-function(fname)
{
print(fname)
fnames<-c(fname[1],fname[2],fname[3],fname[4])
fnames<-sort(fnames)
j<-1
for (i in fnames)
    {
    x_place<-regexpr(patter="x",i)
    num_place<-as.numeric(x_place)+
    newname<-i
    substr(newname,as.character(num_place), as.character(num_place))<-as.character(j)    
...
for (d in subj_dirs) {
  ...
  if (!use_job_array) { 
    if (file.exists(mpragedir)) {
      if (! (file.exists(file.path(mpragedir, paste0("mprage_warpcoef", gradunwarpsuffix, ".nii.gz"))) && file.exists(file.path(mpragedir, "mprage_bet.nii.gz")) ) ) {
      	message("This is the file we are missing: ", paste0("mprage_warpcoef", gradunwarpsuffix, ".nii.gz"))
  ...
  if (useOfflineMB) {
    ...
    ##Note that this is only working for files with BANDIT in the name and with the naming scheme below
    ##Should probably move this to cfg file for generality, but no motivation at the moment.
    if (grepl("Bandit", mb_filepattern, fixed=TRUE)) {
      
      runnums <- sub("^.*ep2d_MB_v409_Bandit_taskx(\\d)_twi(.+)_MB.hdr$",
                     "\\1", mbfiles, perl=TRUE, ignore.case = TRUE)
    ...
    if (length(runnums) > length(unique(runnums))) {
    	#try to change the names if they are the same
    	print(mbfiles)
    	tryCatch(
    		renamedups(mbfiles),
    		error=function(e) stop("Duplicate run numbers detected.")
    	)
    }
  } else {
