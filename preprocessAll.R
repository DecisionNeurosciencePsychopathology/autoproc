...
expectedFuncRuns <- 8 #THIS SHOULD BE PUT IN THE CONFIG
...
mprage_dirs <- list.dirs(pattern="*MPRAGE_axial_32ch_good*") #Should this be generic as well?
...
all_clock_dirs <- list() #think about renaming this variable
for (d in subj_dirs) {
    cat("Processing subject: ", d, "\n")
    setwd(d)

    ##create clock1-clock8 folder structure and copy raw data. LOOK AT THIS IF STATEMENT
    if (!file.exists("MBclock_recon")) { #create MBclock_recon folder if absent THIS WILL NEED TO BE MODIFIED
        dir.create(file.path(d, "MBclock_recon"), showWarnings = FALSE) #AS WILL THIS
    } else {
